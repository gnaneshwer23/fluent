    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700 px-4">
      ...
      <header className="flex flex-col md:flex-row justify-between items-end gap-8">
        <div className="space-y-3">
          ...
          <p className="text-xl text-stone-500 font-serif italic">Synthesized academic nodes and primary research briefs.</p>
        </div>
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded text-[9px] font-bold uppercase tracking-[0.2em] whitespace-nowrap transition-all border ${
              selectedCategory === cat.id 
                ...
                : "bg-white text-stone-500 border-black/5 hover:border-black/10"
            }`}
          >
            {cat.label}
          </button>
            ...
            >
              <Card className="flex flex-col h-full group hover:border-fluent-gold/30 transition-all relative overflow-hidden bg-white p-8">
                ...
                <div className="flex-1 space-y-4">
                  ...
                  <p className="text-sm text-stone-500 leading-relaxed font-serif italic line-clamp-3">
                    {resource.description}
                  </p>
                  <div className="flex flex-wrap gap-3 pt-2">
                    {resource.tags.slice(0, 3).map(tag => (
                      <span key={tag} className="text-[9px] font-display font-medium text-stone-400 uppercase tracking-widest">#{tag}</span>
                    ))}
                  </div>
                </div>
      ...
      {filteredResources.length === 0 && !loading && (
        <div className="py-20 text-center bg-stone-50 rounded-lg border border-dashed border-stone-200">
           ...
           <p className="text-stone-400 font-serif italic">Expand your search parameters or request a new synthesis.</p>
        </div>
      )}
